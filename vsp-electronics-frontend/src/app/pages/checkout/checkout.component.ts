...
export class CheckoutComponent {
  ...
  async submitOrder() {
    ...
    try {
      ...
      // Prepare items for backend (OrderService expects 'items' array)
      const orderItems = cartItems.map(item => ({
        ...
        product: item.product // Include full product if needed by service
      }));
      ...
      const orderData = {
        ...
        paymentMethod: 'cod', // Defaulting to COD as per current flow
        notes: formData.notes,
        userId: this.authService.currentUser()?.id // critical for linking to user account
      };
      ...
      this.orderService.createOrder(orderData).subscribe({
        next: (order) => {
          ...
          if (this.saveCurrentAddress && this.authService.currentUser()) {
            const addressToSave = formData.sameAsShipping ? {
              street: formData.street,
              city: formData.city,
              state: formData.state,
              pincode: formData.pincode,
        ...
        error: (error) => {
          ...
          this.submitError = error.message || 'Failed to process order. Please try again.';
          this.isSubmitting = false;
        }
      });
