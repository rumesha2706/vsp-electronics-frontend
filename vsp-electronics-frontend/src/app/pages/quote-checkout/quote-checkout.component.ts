...
export class QuoteCheckoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.setupPincodeListeners();
    this.states = this.pincodeService.getStates();
  }

  setupPincodeListeners() {
    // Billing Pincode Listener
    this.pincode?.valueChanges.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      filter(value => {
        const isValid = value && value.length === 6 && /^\d{6}$/.test(value);
        if (isValid) console.log('📍 Valid Pincode entered:', value);
        return isValid;
      }),
      switchMap(pincode => {
        console.log('🔄 Fetching details for pincode:', pincode);
        return this.pincodeService.fetchPincodeDetails(pincode);
      })
    ).subscribe({
      next: (data) => {
        if (data) {
          console.log('✅ Pincode details received:', data);
          ...
          console.warn('⚠️ No details found for pincode');
        }
      },
      error: (err) => console.error('❌ Error fetching pincode:', err)
    });

    // Delivery Pincode Listener
    this.deliveryPincode?.valueChanges.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      filter(value => value && value.length === 6 && /^\d{6}$/.test(value)),
      switchMap(pincode => this.pincodeService.fetchPincodeDetails(pincode))
    ).subscribe(data => {
      if (data) {
        this.quoteForm.patchValue({
  ...
  openAddressModal() {
    if (this.pastAddresses.length > 0) {
