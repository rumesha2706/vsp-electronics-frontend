...
export class PincodeService {
  ...
  // Comprehensive Indian pincode database - covering major cities and districts
  private pincodeDatabase: { [key: string]: PincodeData } = {
    // Delhi
   ...
   */
  getLocationByPincode(pincode: string): PincodeData | null {
    return this.pincodeDatabase[pincode] || null;
  }
  ...
  /**
   * Fetch details from API with fallback to local DB
   */
  fetchPincodeDetails(pincode: string): Observable<PincodeData | null> {
    // 1. Check local DB first
    const localData = this.getLocationByPincode(pincode);
    if (localData) {
      return of(localData);
    }

    // 2. Call API
    return this.http.get<any[]>(`${this.apiUrl}/${pincode}`).pipe(
      map(response => {
