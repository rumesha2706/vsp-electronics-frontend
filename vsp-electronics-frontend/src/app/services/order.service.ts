...
export class OrderService {
  ...
  getAllOrders(options: {
    ...
    const { limit = 20, page = 1, status, startDate, endDate, search, paymentStatus, paymentMethod, minTotal, maxTotal } = options;
    const offset = (page - 1) * limit;
    ...
    if (minTotal) params = params.set('minTotal', minTotal.toString());
    if (maxTotal) params = params.set('maxTotal', maxTotal.toString());

    return this.http.get<any>(`${environment.apiUrl}/orders/admin/all`, { params }).pipe(
      map(response => {
        const backendOrders = response.data?.orders || [];
        const orders: Order[] = backendOrders.map((o: any) => ({
          id: String(o.id),
          ...
          items: [], // Map items if backend returns them in list view, or keep empty
          // Helper to parse item count if available
          itemCount: o.item_count ? parseInt(o.item_count) : 0,
          deliveryAddress: {
            street: o.address || '',
            city: o.city || '',
            state: o.state || '',
        ...
        return {
          orders,
          total: response.data?.total || response.data?.pagination?.total || orders.length,
          page,
          limit
        };
      })
   ...
   */
  updateOrderStatus(orderId: string, status: OrderStatus, notifyCustomer: boolean = true): Observable<Order> {
    return this.http.put<any>(`${environment.apiUrl}/orders/${orderId}/status`, {
      status,
      notifyCustomer
    }).pipe(
      map(response => {
        ...
        return {
          ...
          userId: String(o.user_id),
          status: o.status,
          // ... map other fields if needed, or just return partial for UI update
        } as Order;
      })
    );
  }
