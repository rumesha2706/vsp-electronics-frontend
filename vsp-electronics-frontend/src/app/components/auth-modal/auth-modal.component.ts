...
export class AuthModalComponent implements OnInit {
  ...
  onSignup() {
    ...
    this.authService.register(this.signupData).subscribe({
      next: (result: any) => {
        // Backend now handles email sending
        console.log('Registration successful, backend sent email');
    ...
    this.authService.verifyEmail(
      ...
      next: (response) => {
        ...
        setTimeout(() => {
          ...
          this.closeModal();
          this.router.navigate(['/']); // Or reload to refresh session state if needed
        }, 1000);
      },
      error: (error) => {
  ...
  onResendVerificationEmail() {
    ...
    this.authService.resendVerification(this.pendingSignupData.email).subscribe({
      next: () => {
  ...
  onRequestResetCode() {
    ...
    // Check if user exists
    // Check if user exists first (optional, but UI flow does it)
    // Actually authService.forgotPassword handles sending email if user exists
    this.authService.forgotPassword(this.forgotPasswordData.email).subscribe({
      next: (response) => {
        // Backend returns 200 even if user not found for security, but gives message
        this.successMessage = '✅ ' + (response.message || 'Reset code sent if account exists.');
  ...
  onVerifyResetCode() {
    ...
    // Since we are using backend flow where token is part of link or code,
    // and we don't have a verify-only endpoint for password reset code yet,
    // we assume the code entered IS the token needed for resetPassword call.
    this.forgotPasswordData.resetToken = this.forgotPasswordData.code;

    this.successMessage = '✅ Code accepted! Please set your new password.';
